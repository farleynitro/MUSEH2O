...
class SusquehannaModelTest:
    ...
    def res_transition_h(
        ...
        hp = SusquehannaModelTest.g_hydRevCo(
        ...
        # day_of_year, hour0)
        hp_mr = SusquehannaModelTest.g_hydRevMR(
            np.asarray(q_pump),
            np.asarray(q_rel),
            level_Co,
            np.asarray(level_mr),
    ...
    def gini_coefficient_scipy(x_input):
        two_dim_array = SusquehannaModelTest.array_results(x_input)
    ...
    def simulate(
        ...
        for t in range(self.time_horizon_H):
            ...
            if (len(hydropowerProduction_Co) == (self.decisions_per_day * self.n_days_in_year)):
                j_hydro_production_daily_one_year = SusquehannaModelTest.daily_hydropower_average(hydropowerProduction_Co)
        ...
        j_atom_monthly = SusquehannaModelTest.monthly_average(j_atom_yearly_array)
        j_balt_monthly = SusquehannaModelTest.monthly_average(j_balt_yearly_array)
        j_ches_monthly = SusquehannaModelTest.monthly_average(j_ches_yearly_array)
        j_env_monthly = SusquehannaModelTest.monthly_average(j_env_yearly_array)
        ...
        j_rec_monthly = SusquehannaModelTest.g_storagereliability_monthly(j_rec)
        ...
        j_hydro_reliability_yearly_average = SusquehannaModelTest.j_hydro_reliability_energy(hydropower_production_Co_mean, 'yearly')
        ...
        j_hydro_reliability_yearly = SusquehannaModelTest.j_hydro_reliability_energy(j_hydro_production_daily_one_year,
                                                                                 'daily')

        # Now we can calculate the monthly average of hydropower reliability
        j_hydro_monthly = SusquehannaModelTest.monthly_average(j_hydro_reliability_yearly)
        ...
        for i in range(n_months):
            ...
            reliability_euclidean_distance_monthly = SusquehannaModelTest.euclidean_distance_scipy(reliability_monthly)
            reliability_gini_distance_monthly = SusquehannaModelTest.gini_coefficient_scipy(reliability_monthly)
        ...
        eucli_std = SusquehannaModelTest.reliability_std(reliability_eucli)
        gini_std = SusquehannaModelTest.reliability_std(reliability_gini)

        # reliability yearly

        reliability_yearly = [j_atom, j_balt, j_ches, j_env, j_rec, j_hydro_reliability_yearly_average]

        gini_mean = SusquehannaModelTest.gini_coefficient_scipy(reliability_yearly)
        eucli_mean = SusquehannaModelTest.euclidean_distance_scipy(reliability_yearly)
